//! NNTP response codes as defined in RFC 3977 and RFC 4643.
//!
//! This module provides the [`ResponseCode`] enum covering all standard
//! response codes a client may receive from an NNTP server.
//!
//! - **RFC 3977**: Core NNTP protocol response codes (1xx–5xx)
//! - **RFC 4643**: Authentication extension response codes (281, 283, 381, 383, 481, 482)
//!
//! Response codes are grouped by their first digit:
//! - `1xx` — Informational/follow-up responses (multi-line data follows)
//! - `2xx` — Successful command completion
//! - `3xx` — Command accepted, but further action is pending (e.g. password needed)
//! - `4xx` — Transient errors (command was valid, but failed due to current state)
//! - `5xx` — Permanent errors (command invalid or not supported)
...
pub enum ResponseCode {
    ...
    // ─── RFC 4643: Authentication extension ───

    /// 281 - Authentication accepted.
    /// Generated by: AUTHINFO USER, AUTHINFO PASS, AUTHINFO SASL
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "281 - Authentication Accepted")]
    AuthenticationAccepted = 281,

    /// 283 - Authentication accepted (with success data).
    /// Generated by: AUTHINFO SASL
    /// Arguments: 1 (challenge)
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "283 - Authentication Accepted (with success data)")]
    AuthenticationAcceptedWithData = 283,

    /// 381 - Password required.
    /// Generated by: AUTHINFO USER
    /// Meaning: password required via AUTHINFO PASS command.
    /// Note: this code is used for backwards compatibility and does
    /// not conform to the traditional use of 3xx codes.
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "381 - Password Required")]
    ...
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "383 - Continue SASL Exchange")]
    SASLContinue = 383,
    ...
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "481 - Authentication Failed/Rejected")]
    AuthenticationFailed = 481,

    /// 482 - Authentication commands out of sequence.
    /// Generated by: AUTHINFO USER, AUTHINFO PASS, AUTHINFO SASL
    /// Meaning: authentication commands issued out of sequence or SASL protocol error
    /// Multi-line: no
    /// from RFC4643
    #[strum(to_string = "482 - Authentication Commands Out of Sequence")]
